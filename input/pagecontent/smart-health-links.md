...
This guide publishes a self-contained **[clinician viewer](viewer/index.html)** and a worked SMART Health Link that exercises the whole path end to end:
...
- It is encrypted to the direct-file SHL payload published at [`viewer/example.jwe`](viewer/example.jwe) (with `zip:DEF`), and the link descriptor at [`viewer/shl.json`](viewer/shl.json).
- Opening [`viewer/index.html`](viewer/index.html) decrypts that file **in the browser**, runs the application-independent transform, derives the cycle/bleeding/pain/symptom analytics from the granular facts (no precomputed summaries travel in the Bundle), and renders a clinician-facing readout.
